...
type ImgDescription = { num: number; id: number; bs_jitter: number };
...
function generateTimelineVars(
  ...
  const timelineVariables: ImgDescription[] = [];
  ...
  for (let num = 5; num <= 8; num += 1) {
    ...
    for (let i: number = 0; i < nb_blocks; i += 1) {
  ...
  sample: {
    ...
    fn: function (t: number[]): number[] {
      ...
      for (let nums: number = 0; nums < 4; nums++) {
        template = Array.from({length: blocks}, (_, i) => i + nums * blocks);
...
export async function run({
  ...
  let devices: { usb_device: null } = { usb_device: null };

  console.log(`In ExperimentTS input ${input}`);

  const blocks_per_half: number = input.trialsPerHalf || 1;
